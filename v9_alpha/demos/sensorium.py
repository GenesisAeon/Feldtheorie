#!/usr/bin/env python3
"""Sensorium utilities for multi-stream climate cognition.

This module introduces a MultiStreamLoader to weave multiple
real-world (or synthetic) signals into a single interference
pattern for the Lantern network. It follows the Trilayer principle:
structured streams, clear semantics, and narrative-ready summaries.
"""
...
class MultiStreamLoader:
    ...
    Each stream can point to a CSV (value_column required) or
    request a synthetic proxy. Streams are combined via
    σ(β(R-Θ)) = (Σ stress_i) + coupling_factor * Π stress_i,
    capturing both additive pressure and interference.
    """
    ...
    def _synthetic_signal(self, cfg: Dict) -> tuple[np.ndarray, None]:
        ...
        # Add slow drift (R → Θ tension) plus stochastic ζ(R)
