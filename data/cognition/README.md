...
## Adaptive Theta Plasticity Gate Quartet
- Control parameter $R$: normalised sleep-pressure index sampled across staged slow-wave dynamics.
- Critical threshold $\Theta \approx 0.48$: replay ignition point where hippocampal rehearsal surges during consolidation.
- Steepness $\beta \approx 10.86$: shapes a pronounced logistic ascent $\sigma(\beta(R-\Theta))$ from quiescent nights to resonant replay.
- Impedance sketch $\zeta(R) = 1.12 - 0.25\,\sigma$: portrays thalamo-cortical gates relaxing as the choir awakens.

### Provenance & Method
- Synthesised from sleep-dependent memory consolidation literature (Diekelmann & Born 2010) guided by RepoPlan cognition membrane briefs.
- Control parameter checkpoints span 15 synthetic nights seeded with logistic noise to keep reproducibility and cross-domain comparability.
- Processed via `analysis/adaptive_theta_plasticity_fit.py`, which exports $R^2$, AIC, confidence intervals, and dual smooth null falsification to `analysis/results/adaptive_theta_plasticity.json`.

### Files
- `adaptive_theta_plasticity.csv`: sleep-pressure $R$ and replay probability $\sigma$.
- `adaptive_theta_plasticity.metadata.json`: descriptor with $(R,\Theta,\beta,\zeta)$ estimates, falsification margins, and ethical annotations.
- `../../analysis/results/adaptive_theta_plasticity.json`: logistic fit and null comparisons generated by the analysis pipeline.

### Falsifiability Ledger (see metadata)
- Logistic dawn exceeds both linear and power-law null breezes with $\Delta \mathrm{AIC} > 30$ and non-negative $\Delta R^2$ margins.
- Confidence corridors on $(\Theta, \beta)$ keep replay thresholds auditable for downstream manuscripts and simulator presets.

### Cognitive Resonance
Sleep-pressure gating is rendered as a nocturnal membrane: below $\Theta$ the hippocampal choir whispers, the impedance $\zeta(R)$ holding rehearsal in check. As nights ease across the threshold, $\beta$ sculpts a rapid awakening, $\zeta(R)$ slackens toward 0.88, and the dawn chorus of replay floods the cognition grove—primed for cross-links into `docs/`, the simulator presets, and ethical guardrails for sleep research metaphors.
