...
1. **UTAC Status Matrix** (`docs/utac_status_alignment_v1.2.md#metaquest-handshake`)
   keeps the ΔAIC guard visible whenever automation pushes R forward.
2. **Metaquest Parity Brief** (`docs/metaquest_parity_brief.md`) lists
   mq-parity-001…004, the gate that any automation change must salute.
3. **BreakPoint Rituals** (`seed/BreakPointAnalyse/WayToGo.txt` &
   `ReaktionWayToGo.txt`) script the restart path should ζ(R) spike.
4. **Automation Hooks** (`scripts/archive_sigillin.py`, Makefile targets) confirm
   Ordnungs-Sigillin stay synchronised.
5. **Codex Ledger** (`seed/codexfeedback.*`) is the mycelium that records every
   mq-sys vector.
...
- **mq-sys-vector-003 – Falsifiability narrative.** Expand the compass and codex
  notes with ΔAIC/null comparisons for Metaquest automation vs. baseline.
