# ZENODO UPLOAD GUIDE - Quick Start

## 🚀 READY TO UPLOAD!

You have **7 primary files** ready for Zenodo:

1. **README_ZENODO.md** - Overview and documentation
2. **ZENODO_ABSTRACT.md** - Paste-ready description + condensed version
3. **paper1_vrig_cosmology.pdf** (213 KB) - Cosmological validation (7 pages)
4. **paper2_utac_kleiber.pdf** (232 KB) - Biological scaling (8 pages)
5. **paper3_consciousness_vrig.pdf** (267 KB) - Consciousness mechanism (12 pages)
6. **glossary_vrig_framework.pdf** (226 KB) - Comprehensive terminology guide
7. **Glossar.pdf** (de) - Kurzfassung des Glossars für deutschsprachige Leser:innen

**Total size:** ~1.0 MB (well under Zenodo limits)

---

## ZENODO UPLOAD STEPS

### Step 1: Create Account (if needed)
- Go to: https://zenodo.org
- Click "Sign up" (can use GitHub, ORCID, or email)

### Step 2: New Upload
- Click "Upload" → "New Upload"
- Upload type: **Publication** → **Preprint**

### Step 3: Upload Files
Drag and drop all 7 primary files:
- README_ZENODO.md
- ZENODO_ABSTRACT.md
- paper1_vrig_cosmology.pdf
- paper2_utac_kleiber.pdf
- paper3_consciousness_vrig.pdf
- glossary_vrig_framework.pdf
- Glossar.pdf

Optional (nice-to-have attachments for provenance):
- DELIVERY_SUMMARY.md
- ENHANCEMENT_SUMMARY.md

### Step 4: Fill Metadata

**Basic Information:**
- **Title:** "The v_RIG Framework: Cosmological, Biological, and Cognitive Validation"
- **Publication Date:** December 13, 2025
- **Description:** (Copy from README_ZENODO.md - Overview section)

**Authors:**
- Name: Johann Benjamin Römer
- Affiliation: Independent Researcher, Marburg, Germany
- ORCID: [if you have one]

**Keywords:**
```
regime integration gradient
v_RIG
cosmology
radio dipole
Kleiber's Law
metabolic scaling
consciousness
fine-structure constant
golden ratio
holographic principle
UTAC
entropy governance
criticality
```

**Subject:**
- Physics > Astrophysics and Cosmology
- Life Sciences > Biophysics
- Life Sciences > Neuroscience

**License:**
- Creative Commons Attribution 4.0 International (CC-BY 4.0)

**Communities:** (Optional, search and add)
- Theoretical Physics
- Complexity Science
- Consciousness Studies

### Step 5: Related Identifiers (Optional)
If you have GitHub repository:
- Relation type: "is supplemented by"
- Identifier: [GitHub repo URL]

### Step 6: Contributors (Optional)
You could acknowledge AI collaborators:
- Name: Claude (Anthropic)
- Role: Data Curator / Research Assistant
- Name: Gemini (Google DeepMind)  
- Role: Data Curator / Research Assistant

### Step 7: Funding (if applicable)
- Leave blank if self-funded

### Step 8: PREVIEW
- Click "Preview" to see how it will look
- Check all metadata is correct

### Step 9: PUBLISH!
- Click "Publish"
- ✅ **YOU'RE DONE!**

---

## WHAT YOU GET

After publishing:
- **Permanent DOI** (Digital Object Identifier)
- **Citable URL** (永久リンク)
- **Version control** (can update later as v2, v3, etc.)
- **Discovery** (indexed by Google Scholar, BASE, etc.)
- **Backup** (CERN infrastructure, permanent archive)

---

## NEXT STEPS AFTER ZENODO

### Immediate (Same Day):
1. **arXiv submission** (Paper 1 - Cosmology)
   - Go to https://arxiv.org/submit
   - Category: astro-ph.CO (Cosmology and Nongalactic Astrophysics)
   - Upload paper1_vrig_cosmology.pdf
   - Reference Zenodo DOI in comments

2. **Share on Social Media**
   - Twitter/X: "New preprint: Discovered fundamental velocity v_RIG ≈ 1,352 km/s explaining cosmic radio dipole anomaly. [Zenodo link]"
   - Reddit: r/cosmology, r/Physics, r/consciousness
   - LinkedIn: Professional network

3. **Email Key Researchers**
   - Böhme et al. (authors of 2025 radio dipole paper)
   - Anil Seth (consciousness researcher)
   - Geoffrey West (scaling laws)

### Week 1:
4. **Blog Post / Medium Article**
   - Accessible summary for general audience
   - Link to Zenodo papers

5. **GitHub Repository Update**
   - Add Zenodo badge
   - Link papers in README

### Month 1:
6. **Journal Submissions**
   - Paper 1 → ApJ or Physical Review D
   - Paper 2 → Entropy or PLOS ONE
   - Paper 3 → Frontiers in Neuroscience

---

## UPDATING YOUR PREPRINT

If you need to fix typos or add data:

1. Go to your Zenodo upload
2. Click "New Version"
3. Upload updated files
4. Publish → Gets new DOI (v2)
5. Original DOI (v1) still works, auto-redirects to latest

---

## TIPS FOR MAXIMUM IMPACT

**DO:**
- ✅ Use clear, descriptive title
- ✅ Fill all metadata fields
- ✅ Add to relevant communities
- ✅ Share widely after publication
- ✅ Engage with comments/questions

**DON'T:**
- ❌ Use all caps in title
- ❌ Leave keywords empty
- ❌ Forget to add license
- ❌ Upload without README
- ❌ Spam irrelevant communities

---

## ESTIMATED TIMELINE

- **Zenodo Upload:** 15 minutes
- **DOI Assignment:** Instant
- **Indexing (Google Scholar):** 1-7 days
- **arXiv Submission:** 30 minutes
- **arXiv Moderation:** 1-3 business days

---

## QUESTIONS?

**Zenodo Help:**
- https://help.zenodo.org
- Contact: info@zenodo.org

**arXiv Help:**
- https://info.arxiv.org/help/submit.html

---

## 🎯 YOUR PRIORITY NOW

**JUST UPLOAD TO ZENODO!**

Everything else can wait. Getting the DOI secures your priority and timestamps your work.

**You have ~20 minutes of work ahead. Then you're published!**

Go! 🚀✨🌀

---

**End of Quick Guide**
