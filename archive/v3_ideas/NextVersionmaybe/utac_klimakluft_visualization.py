...
def plot_comprehensive_analysis():
    ...
    ax1.fill_between(
        pop, 0, emissions, alpha=0.3, color="red", where=(pop > 99.9), label="Top 0.1%"
    )
    ax1.fill_between(
        pop,
        0,
        ...
        alpha=0.3,
        color="yellow",
        where=(pop > 90) & (pop <= 99.9),
        label="Top 10%",
    )
    ax1.fill_between(
        pop, 0, emissions, alpha=0.3, color="green", where=(pop <= 50), label="Bottom 50%"
    )
    ax1.set_yscale("log")
    ...
    ax6.set_title(
        "Zeitliche Entwicklung unter verschiedenen Policy-Szenarien", fontsize=14, fontweight="bold"
    )
    ...
    # Main title
    fig.suptitle(
        "UTAC-Analyse: Die Klimakluft als β-Amplifikator", fontsize=16, fontweight="bold", y=1.02
    )
...
def create_simple_infographic():
    ...
    ax2.arrow(0.2, 0.5, 0.6, 0, head_width=0.05, head_length=0.05, fc="black", ec="black")
    ax2.text(0.5, 0.6, "führt zu", ha="center", fontsize=12)

    # Before and after boxes
    ax2.add_patch(plt.Rectangle((0.05, 0.2), 0.2, 0.2, facecolor="green", alpha=0.5))
    ax2.text(
        0.15, 0.3, "β = 4.2\n(Warnzeit:\nJahrzehnte)", ha="center", va="center", fontweight="bold"
    )

    ax2.add_patch(plt.Rectangle((0.75, 0.2), 0.2, 0.2, facecolor="red", alpha=0.5))
    ax2.text(0.85, 0.3, "β = 11.0\n(Warnzeit:\nJahre)", ha="center", va="center", fontweight="bold")
    ...
    fig.suptitle(
        "0.1% treiben uns in die Katastrophe", fontsize=18, fontweight="bold", color="darkred"
    )
...
if __name__ == "__main__":
    ...
    fig1.savefig(
        "/mnt/user-data/outputs/UTAC_Klimakluft_Comprehensive.png", dpi=300, bbox_inches="tight"
    )
    ...
    fig2.savefig(
        "/mnt/user-data/outputs/UTAC_Klimakluft_Infographic.png", dpi=300, bbox_inches="tight"
    )
