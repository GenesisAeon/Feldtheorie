...
class DataValidator:
    ...
    def _check_utac_consistency(self, rows: list[dict]):
                ...
                if "beta" in row and row["beta"]:
                    ...
                    if beta < BETA_RANGE[0] or beta > BETA_RANGE[1]:
                        self.warnings.append(
                            f"Row {i}: β={beta:.2f} outside typical range {BETA_RANGE}"
                        )
                ...
                if theta != 0:
                    ...
                    if ratio < R_THETA_RATIO_RANGE[0] or ratio > R_THETA_RATIO_RANGE[1]:
                        if abs(r - theta) / theta > 0.5:  # More than 50% deviation
                            self.warnings.append(
                                f"Row {r_idx}: R/Θ ratio={ratio:.2f} may indicate pre/post-tipping state"
                            )
        ...
        if beta_values:
            ...
            print(
                f"  ✓ β-range: [{min(betas):.2f}, {max(betas):.2f}], mean: {sum(betas)/len(betas):.2f}"
            )
