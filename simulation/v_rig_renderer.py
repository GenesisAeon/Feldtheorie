...
class VRigRealityRenderer:
    ...
    def find_coherence_peak(self) -> tuple[int, float]:
        ...
        N_values, coherence_values = self.scan_window_sizes(
            N_min=int(self.target_n * 0.5), N_max=int(self.target_n * 1.5), N_step=5
        )
        ...
        print(
            f"   Deviation: {abs(optimal_N - self.target_n):.1f} ({abs(optimal_N - self.target_n) / self.target_n * 100:.1f}%)"
        )
