...
class SigillinAlertSystem:
    ...
    def __init__(self, alerts_path: str = None, output_dir: str = None):
        ...
        if alerts_path is None:
            alerts_path = (
                Path(__file__).parent.parent / "analysis" / "results" / "ews_pipeline_alerts.json"
            )

        if output_dir is None:
            output_dir = (
                Path(__file__).parent.parent.parent / "seed" / "FraktaltagebuchV3" / "alerts"
            )
    ...
    def create_alert_document(self, alert: dict, timestamp: str) -> dict:
        ...
        formal_thread = {
            ...
            "trend": (
                {"tau": alert.get("trend_tau"), "p_value": alert.get("trend_p")}
                if alert.get("trend_tau") is not None
                else None
            ),
        }
...
"""
        ...
        if formal.get("trend"):
            tau_val = (
                f"{formal['trend']['tau']:.3f}" if formal["trend"]["tau"] is not None else "N/A"
            )
            p_val = (
                f"{formal['trend']['p_value']:.4f}"
                if formal["trend"]["p_value"] is not None
                else "N/A"
            )
            md_content += f"""
### Trend Analysis
...
"""
        ...
        if empirical["measurements"].get("exceedance"):
            md_content += (
                f"| **Exceedance Factor** | {empirical['measurements']['exceedance']:.2f}x |\n"
            )
