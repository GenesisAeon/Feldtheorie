...
def create_powerlaw_export():
    ...
    fieldnames = [
        'domain', 'beta', 'beta_ci_lower', 'beta_ci_upper', 'beta_ci_width',
        'theta', 'r_squared', 'delta_aic', 'source',
        'domain_cluster', 'phi_attractor', 'phi_value', 'phi_distance',
        'phi_match_percent', 'alpha_hypothetical_inverse', 'alpha_hypothetical_scaled'
    ]
    ...
    alpha_fieldnames = ['parameter', 'alpha_value', 'description', 'formula',
                       'source', 'beta_inverse', 'beta_scaled']
    ...
    for name, data in attractors.items():
        match_error = abs(
            (data['beta_empirical_mean'] - data['beta_target']) /
            data['beta_target'] * 100
        )
        phi_records.append({
            'attractor': name,
    ...
    phi_fieldnames = ['attractor', 'phi_value', 'formula', 'domain',
                     'beta_target', 'beta_empirical_mean', 'beta_empirical_sd',
                     'match_error_percent']
    ...
    for record in alpha_records:
        print(f"  {record['parameter']}: {record['alpha_value']} "
              f"(β_inverse={record['beta_inverse']})")

    print(f"\nΦ-Attractors (UTAC v2.0):")
    for record in phi_records:
        print(f"  {record['attractor']}: {record['phi_value']} "
...
if __name__ == "__main__":
    ...
    print(f"  • powerlaw_parameters_utac_analysis.metadata.json")
    print(f"\nLocation: {OUTPUT_DIR}")
