...
# Import our models
from cosmic_alpha_phi import BOEHME_UNCERTAINTY_KM_S, BOEHME_VELOCITY_KM_S, CosmicQuantization
...
def plot_cosmic_quantization(ax):
    ...
    ax.set_title(
        "A. Cosmic Velocity Scaling: α-Φ Formula Test", fontsize=TITLE_SIZE, fontweight="bold"
    )
...
def plot_social_phase_transition(ax):
    ...
    # Main rigidity curve
    ax.plot(
        transition.gini_values,
        rigidity_values,
        linewidth=3,
        color=COLOR_THEORY,
        label=r"$\beta(Gini) = Gini \cdot Load$",
    )
    ...
    if np.any(frozen_mask):
        ax.fill_between(
            ...
            0,
            rigidity_values[frozen_mask],
            alpha=0.2,
            color=COLOR_FROZEN,
            label="Frozen (locked-in)",
        )
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=2.5,
        label=f"Critical Point (Gini = {critical_gini:.2f})",
        alpha=0.8,
    )
    ax.plot(
        critical_gini,
    ...
    # Add warning zone
    ax.axhspan(1.5, 3.0, alpha=0.15, color=COLOR_FROZEN, zorder=0)
    ax.text(
        0.22,
        2.2,
        "DANGER ZONE\n(β → ∞)",
    ...
    ax.set_title(
        "B. Social Dynamics: Ising Model with Inequality", fontsize=TITLE_SIZE, fontweight="bold"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--show", action="store_true", help="Display figure interactively (in addition to saving)"
    )
