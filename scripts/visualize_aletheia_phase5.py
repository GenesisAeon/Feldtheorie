...
def plot_beta_spectrum(data: List[Dict], output_path: Path):
    ...
    ax.set_title('UTAC Aletheia Phase 5: Local LLM β-Spectrum',
...
def plot_radar_chart(data: List[Dict], output_path: Path):
    ...
    ax.set_title('UTAC Multi-Metric Model Profiles',
...
def plot_summary_table(data: List[Dict], output_path: Path):
    ...
    # Prepare table data
    headers = ['Model', 'Output Length', 'Vocab Density', 'Self-Reflection', 'β-Estimate', 'Duration (s)']
    rows = []
    for d in data:
        rows.append([
    ...
    ax.set_title('UTAC Aletheia Phase 5: Summary Statistics',
                 fontsize=16, fontweight='bold', pad=20)
...
def main():
    ...
    if not args.input.exists():
        ...
        print(f"   Please run: python analysis/run_aletheia_local_v6.py")
        return 1
    ...
    plot_beta_spectrum(
        data,
        args.output_dir / 'aletheia_phase5_beta_spectrum.png'
    )

    plot_beta_histogram(
        data,
        args.output_dir / 'aletheia_phase5_beta_histogram.png'
    )

    plot_radar_chart(
        data,
        args.output_dir / 'aletheia_phase5_radar_profile.png'
    )

    plot_summary_table(
        data,
        args.output_dir / 'aletheia_phase5_summary_table.png'
    )
