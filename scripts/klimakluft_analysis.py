...
class KlimaklustAnalyzer:
    ...
    def visualize_beta_bomb(self, save_path="klimakluft_beta_bomb.png"):
        ...
        axes[0, 0].axhline(
            y=2, color="green", linestyle="--", linewidth=2, label="Sustainable (~2 kg/day)"
        )
        axes[0, 0].axhline(
            y=800, color="red", linestyle="--", linewidth=2, label="0.1% Peak (>800 kg/day)"
        )
        ...
        axes[0, 1].axhline(
            y=11, color="red", linestyle="--", linewidth=2, label="β = 11 (catastrophic)"
        )
        axes[0, 1].fill_between(
            ...
            color="red",
            alpha=0.2,
            label="Amplified zone",
        )
        axes[0, 1].axvline(
            x=0.72, color="orange", linestyle=":", linewidth=2.5, label="Current G_CO₂ ≈ 0.72"
        )
        ...
        axes[1, 0].axhline(
            y=50, color="green", linestyle="--", linewidth=2, label="Baseline (50 yr)"
        )
        axes[1, 0].axhline(y=5, color="red", linestyle="--", linewidth=2, label="Collapsed (5 yr)")
        axes[1, 0].fill_between(gini_range, 0, tau_curve, alpha=0.3, color="lightblue")
        axes[1, 0].axvline(
            x=0.72, color="orange", linestyle=":", linewidth=2.5, label="Current G_CO₂"
        )
        axes[1, 0].set_xlabel("Emission Gini Coefficient", fontsize=11)
        axes[1, 0].set_ylabel("τ_warning (years)", fontsize=11)
        axes[1, 0].set_title(
            "C) Early Warning Time Collapse (AMOC)", fontsize=12, fontweight="bold"
        )
        ...
        axes[1, 1].axhline(
            y=50, color="black", linestyle=":", alpha=0.6, linewidth=2, label="50% threshold"
        )
        ...
        plt.suptitle(
            "The 0.1% Beta-Bomb: Emission Inequality Drives Climate Criticality",
            fontsize=15,
            fontweight="bold",
            y=0.998,
        )
...
def main():
    ...
    # Initialize analyzer
    analyzer = KlimaklustAnalyzer(beta_base=4.5, variance_coupling=5.0, political_impedance=1.75)
    ...
    print(
        f"   • BAU β-amplification: {analyzer.beta_base:.1f} → {bau_beta:.1f} "
        + f"({(bau_beta/analyzer.beta_base - 1)*100:.0f}% increase)"
    )
    print(
        f"   • Warning time collapse: {rad_tau:.0f} yr → {bau_tau:.0f} yr "
        + f"({(1 - bau_tau/rad_tau)*100:.0f}% reduction)"
    )
    print(
        f"   • Radical deconcentration recovers: β = {rad_beta:.1f} "
