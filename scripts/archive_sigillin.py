...
class SigillinArchiver:
    ...
    def _load_archive_index(self):
        ...
        if self.archive_index_yaml.exists():
            with open(self.archive_index_yaml, encoding="utf-8") as f:
    ...
    def _save_archive_index(self):
        ...
        with open(self.archive_index_yaml, "w", encoding="utf-8") as f:
            yaml.dump(
                self.archive_index, f, allow_unicode=True, sort_keys=False, default_flow_style=False
            )
...
"""
    ...
    def archive_sigillin(self, sigillin_path):
        ...
        if not needs_archiving:
            print(
                f"✅ {path.name} is within limits (Size: {info['size_kb']:.2f}KB, Entries: {info.get('entry_count', 'N/A')})"
            )
            return False

        print(
            f"⚠️  Exceeds limits: Size={info['size_kb']:.2f}KB (max {self.max_size_kb}), Entries={info.get('entry_count', 'N/A')} (max {self.max_entries})"
        )
        ...
        print(
            f"📊 Total entries: {len(entries)} | Archiving: {len(old_entries)} | Keeping: {len(recent_entries)}"
        )
        ...
        with zipfile.ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            if path.suffix in [".yaml", ".yml"]:
                content = yaml.dump(
                    archived_data, allow_unicode=True, sort_keys=False, default_flow_style=False
                )
            else:
        ...
        print(
            f"✅ Created archive: {archive_filename} ({compressed_size_kb:.2f}KB, {compression_ratio:.1f}% of original)"
        )
    ...
    def _recount_docs(self, timestamp: str) -> dict:
        ...
        self._log_index_delta(
            "docs", filesystem_count, len(listed_set), missing_files, orphan_entries
        )
    ...
    def _recount_analysis(self, timestamp: str) -> dict:
        ...
        self._log_index_delta(
            "analysis", filesystem_count, len(listed_set), missing_files, orphan_entries
        )
    ...
    def _recount_models(self, timestamp: str) -> dict:
        ...
        self._log_index_delta(
            "models", filesystem_count, len(listed_set), missing_files, orphan_entries
        )
...
def main():
    ...
    parser.add_argument(
        "--recount", action="store_true", help="Refresh index metadata counts and emit summary"
    )
    parser.add_argument(
        "--recount-targets",
        type=str,
