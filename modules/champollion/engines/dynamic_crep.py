...
class DynamicMetricEngine:
    ...
    def _find_config(self) -> Path:
        ...
        raise FileNotFoundError(
            f"Could not find sigillin_core_definition.yaml in: {search_paths}"
        )
    ...
    def calculate_all_metrics(self, aggregated_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_key, metric_def in self.metrics.items():
            ...
            if not algorithm_func:
                print(f"⚠️  Warning: Algorithm '{algorithm_name}' not found for metric '{metric_key}'")
                continue
            ...
            try:
                ...
                results['metrics'][metric_key] = {
                    ...
                    'threshold_status': self._check_threshold(value, metric_def.get('thresholds', {}))
                }
...
def calculate_folder_sigillin(aggregated_data: Dict[str, Any],
...
if __name__ == '__main__':
    ...
    dummy_data = {
        ...
        'subfolder_contexts': [
            {'folder': 'subfolder1', 'summary': 'Test 1', 'confidence': 0.88, 'total_items': 5, 'keywords': ['test', 'data']},
            {'folder': 'subfolder2', 'summary': 'Test 2', 'confidence': 0.92, 'total_items': 7, 'keywords': ['test', 'analysis']}
        ],
        'origins': {'empirical': 2},
