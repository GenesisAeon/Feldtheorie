...
ANOVA reveals significant domain variation ($F = 3.21$, $p = 0.008$), but 9/11 domains cluster around $\beta = 4.2 \pm 0.8$. Outliers: Climate (mean $= 8.4$, high-$\beta$ cascades) and Materials (mean $= 2.5$, quantum effects). Excluding outliers: $R^2 = 0.782$, mean $\beta = 4.09 \pm 0.15$ (Fig.~\ref{fig:outliers}).

\textbf{Field Type Analysis:} Incorporating field type classification (Types I-IV: Strongly Coupled, High-Dimensional, Weakly Coupled, Physically Constrained) as categorical predictors substantially improves model fit. Type-conditioned regression yields adjusted $R^2 = 0.665$ with field type ANOVA showing strong effect ($\eta^2 = 0.468$, $p < 0.001$). Type IV (Physically Constrained) systems exhibit highest $\beta$ values (urban heat: 16.3, amazon moisture: 14.6), while Type III (Weakly Coupled) shows lowest ($\beta \approx 2.5$) (Fig.~\ref{fig:fieldtypes}).
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figure6_beta_by_field_type.png}
\caption{\textbf{$\beta$ Distribution by Field Type.} Box plots showing $\beta$ distribution across four field types (n=15 validated systems). Type I (Strongly Coupled, blue): Climate systems with moderate $\beta \approx 4.0$. Type II (High-Dimensional, purple): Social systems with $\beta \approx 3.5$. Type III (Weakly Coupled, orange): Neural plasticity with low $\beta \approx 2.5$. Type IV (Physically Constrained, red): Urban heat and moisture recycling with high $\beta > 10$. Field type classification explains 46.8\% of $\beta$ variance ($\eta^2 = 0.468$, $p < 0.001$).}
\label{fig:fieldtypes}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figure7_beta_outlier_analysis.png}
\caption{\textbf{Outlier Analysis and Robustness.} Diagnostic plots for meta-regression residuals showing (A) Leverage vs. standardized residuals identifying high-$\beta$ climate systems (urban heat, amazon moisture) as influential outliers. (B) Cook's distance analysis confirming these systems have strong leverage but do not distort overall model fit (D < 0.5 threshold). (C) Quantile-quantile plot showing residuals follow approximate normality except at extreme tails. Excluding outliers improves $R^2$ from 0.665 to 0.782, validating robustness of core $\beta \approx 4.2$ convergence across remaining systems.}
\label{fig:outliers}
\end{figure}
