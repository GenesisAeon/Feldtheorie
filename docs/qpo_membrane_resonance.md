...
Feeding the trajectory into `analysis/resonance_fit_pipeline.py --mode ingest` yields [`analysis/reports/qpo_membrane_summary.json`](../analysis/reports/qpo_membrane_summary.json). The logistic fit recovers \(\Theta\) and \(\beta\) within tight 95% intervals while delivering \(R^2 = 1.0\) and a dramatically lower AIC than the smooth linear null (\(\Delta \mathrm{AIC} \approx 2.48\times 10^4\), \(\Delta R^2 \approx 0.148\)). This falsification of the null lullaby documents how the membrane's resonance chorus outruns any monotonic drift, satisfying the repository mandate for explicit null comparisons.
