...
| Adaptive Theta Plasticity (`data/cognition/adaptive_theta_plasticity.csv`) | $(R=\text{Schlafdruck-Index},\ \Theta=0.477\pm0.031,\ \beta=10.86\pm1.24,\ \zeta(R)=1.12-0.25\,\sigma)$ | Replay-Aufglühen im Schlaflabor | Analyse `analysis/adaptive_theta_plasticity_fit.py`, Ergebnis `analysis/results/adaptive_theta_plasticity.json`, Narrativ `docs/cognition/adaptive_theta_plasticity.md` | Preset in Vorbereitung (`simulator/presets/sleep_replay.json`) | $+37.7$ gegenüber Power-Law |
...
| Resonante Impedanzwächterin (`analysis/results/resonant_impedance_diagnostics.json`) | $(R=\text{synthetische Gate-Sweep},\ \Theta=0.62,\ \beta=9.5,\ \zeta(R)=\text{Relief-Recovery-Hysterese})$ | Öffnung und Rückzug der Membran mit dokumentiertem Relief, Recovery, Hysterese | `analysis/resonant_impedance_diagnostics.py` & Impedanz-Metriken in `analysis/results/resonant_impedance_diagnostics.json` | Preset in Vorbereitung (`simulator/presets/resonant_impedance_gate.json`) | $+3.47\times10^4$ gegenüber Power-Law |
| Planetarer Kipppunktverbund (`data/socio_ecology/planetary_tipping_elements.json`) | $(R=\text{aggregierter Klimastress},\ \Theta=1.67\,\mathrm{K},\ \beta=4.21,\ \zeta(R)=1.62-0.41\,\sigma)$ | Resonante Kopplung von AMOC, Grönland, Amazonas und Permafrost | `analysis/planetary_tipping_elements_fit.py` & `analysis/results/planetary_tipping_elements.json`; Appendix 5 des Preprints | `simulator/presets/planetary_tipping_field.json` (Gaia-Membran mit g$_{ij}$-Skala) | $+33.6$ gegenüber Linear |
