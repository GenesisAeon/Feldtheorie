...
1. **Safety-Delay Field Prototype:** `simulation/safety_delay_field.py` now couples with `analysis/safety_delay_sweep.py`, which exports `analysis/results/safety_delay_sweep_20251107T202620Z.json` documenting τ_delay, β-shift, control energy and ΔAIC vs linear & constant nulls. Next step: route these diagnostics into `simulator/cli.py` presets and seed the ΔAIC(t) ledger in `data/safety_delay/`.
...
| ---- | ---------------- | -------------- | -------------- |
| Safety-Delay τ* modelling | `simulation/`, `analysis/` | ✅ `analysis/safety_delay_sweep.py` + `analysis/results/safety_delay_sweep_20251107T202620Z.json`; next: couple with `simulator/cli.py`, log ΔAIC(t) in `data/safety_delay/` | Compare against baseline logistic sweep, cite Θ(t) envelopes |
