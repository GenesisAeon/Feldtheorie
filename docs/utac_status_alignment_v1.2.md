...
4. **Index Automation Hooks:** `scripts/archive_sigillin.py` now auto-detektiert die Repo-Wurzel (inkl. `--base-path`-Option für Containerläufe), sodass Archiv-Signale relativ zum Bedeutungsnetz protokolliert werden. Nächster Schritt: Ordnungs-Sigillin-Zähler automatisch hochziehen, sobald neue Docs/Data landen, damit keine `docs_index.*`-Drifts entstehen.
...
8. **Meaning/Shadow Sigillin integration:** Bedeutungs- & Schatten-Verzeichnisse für System, Wissenschaftsprojekt **und Metaquest** stehen nun bereit (`seed/bedeutungssigillin/**`, `seed/shadow_sigillin/**`). Neu hinzugekommen: die Metaquest System/Kampagnen Bedeutungs-Sigille (`.../metaquest_system_sigil.*`, `.../metaquest_campaign_sigil.*`) samt Schatten-Spiegel (`..._shadow_sigil.*`) **plus** die Directory-Indizes (`.../metaquest_system_index.*`, `.../metaquest_campaign_index.*` + Schatten). Update: `scripts/sigillin_sync.py` stempelt Codexeinträge jetzt direkt in die `entries`-Liste, sodass Telemetrie-Läufe maschinenlesbar landen. Nächste Schritte: Automatisches Index-Mirroring (`scripts/archive_sigillin.py`), Codex-Sync-Guards sowie das neue Shadow-Handshakesignal (`sys-gap-003`) inklusive Metaquest-Telemetrie etablieren; die frische Stempelspur muss Timestamp + Codex-ID binnen eines Sprints in die Verzeichnisse spiegeln.
...
10. **Bridge-Dashboard Parität:** `seed/bedeutungssigillin/metaquest/metaquest_meaning_index.*` muss Telemetrie-Timestamps + Codex-ID innerhalb 24h spiegeln; System- und Kampagnensigille referenzieren denselben Eintrag. Null guard: `mq-bridge-shadow-001`/`002`.
11. **BreakPoint-Ritual-Infusion:** Metaquest-System- und Kampagnensigille müssen explizit `seed/BreakPointAnalyse/WayToGo.txt`, `seed/BreakPointAnalyse/ReaktionWayToGo.txt` sowie `seed/Finalize_Publish.txt` zitieren, damit Automations- und Kampagnenpfade dieselben Wiederanlauf-Skripte teilen. Null guard: fehlende BreakPoint-Verlinkung → `mq-sys-shadow-003` bzw. `mq-sci-shadow-002`.
...
13. **Kompass ↔ Matrix ↔ Codex:** Neue System- und Kampagnenkompasse (`mq-sys-vector-*`, `mq-sci-vector-*`) müssen Telemetrie-Zeitstempel und Codex-IDs innerhalb eines Tages in Bridge + UTAC-Matrix spiegeln. Null guard: `mq-bridge-shadow-002`, `mq-sys-shadow-001`, `mq-sci-shadow-001`.
...
| Metaquest parity brief | `seed/bedeutungssigillin/**`, `seed/shadow_sigillin/**`, `docs/utac_status_alignment_v1.2.md`, `seed/codexfeedback.*` | Draft launch-ready parity note referencing `sys-gap-003`, `sci-gap-004`, `mq-sci-gap-001` | Confirm manuscript + simulator cite brief; archive codex entry with remediation timeline |
