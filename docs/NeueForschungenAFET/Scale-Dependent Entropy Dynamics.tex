...
\begin{itemize}[leftmargin=1.8em]
  ...
  \item \textbf{UTAC-v2} (iterations 20--38, current): the expanded 78-system analysis
        revealed domain-specific $\beta$ clustering. This is not a contradiction but a
        scientific refinement: universality lies in the \emph{architecture of clustering},
        not in a single $\beta$ value.
\end{itemize}
...
The empirical core --- domain-clustered $\beta$ values across 78 systems, with $\eta^2 = 0.91$ ---
is statistically robust and independently reproducible.
...
% ===================================================================
\section{Provenance and Development Path}
\label{sec:provenance}

This section documents the reconstruction of the AFET/UTAC development chronology
based on Git forensics across two repositories (\texttt{unified-mandala} and
\texttt{GenesisAeon/Feldtheorie}) and archived conversation logs.
It is presented as rekonstruktive Dokumentation --- not as self-legitimation ---
following the principle that transparency about origins strengthens, rather than
weakens, a research programme.

\subsection*{Methodological Note}

Both repositories have large initial import commits
(\texttt{4ae34d7}, 2025-09-15 in \texttt{unified-mandala};
\texttt{fdfd158}, 2026-01-25 in \texttt{Feldtheorie}).
All ``first seen'' dates therefore represent \emph{the earliest verifiable
repository timestamp}, not necessarily the absolute origin of the concept.
Where pre-repository origins are claimed, they rest on narrative reconstruction
from archived dialogues, not on independent timestamps.

\subsection*{Provenance Table}

\begin{longtable}{p{2.8cm}p{2.2cm}p{2.2cm}p{5.2cm}}
\caption{Provenance of core AFET/UTAC concepts.
Columns: Concept $|$ First repo commit $|$ Claim type $|$ Notes.}
\label{tab:provenance}\\
\toprule
\textbf{Concept} & \textbf{First commit / date} & \textbf{Claim type} & \textbf{Notes} \\
\midrule
\endfirsthead
\multicolumn{4}{l}{\small(continued)}\\
\toprule
...
\midrule
\endhead
Sigmoid threshold logic & \texttt{4ae34d7} / 2025-09-15 & Human-origin & Mandala Climate Dashboard;
  UTAC v1 precursor; Sigmoid in NN modules + CREP adaptive thresholds. \\[4pt]

$\beta = 4.2$ (universal) & not timestamp-belegbar & LLM-generated (Gemini) &
  Derived from 5 datasets used as calculation basis, not as derivation source.
  Subsequently corrected through iterative expansion to 78 systems. \\[4pt]

$\beta$-domain hierarchy & Zenodo iter.\ 20--38 & Iterative refinement &
  UTAC-v1 ($\beta$ universal) corrected to UTAC-v2 (domain clusters)
  via expansion from 5 to 78 systems. ANOVA result emerged from data,
  not assumed. \\[4pt]

OIPK framework & \texttt{fdfd158} / 2026-01-25 & Human-origin &
  Orthogonal Implosion-Photon-Kinematik; internal model for 3D volumetric
  unfolding with photonic time axis. Precursor to formal $\vrig$ derivation.
  Note: ``OPIK'' in narrative texts is a transcription error for OIPK. \\[4pt]

$\vrig \approx 1352$\,km\,s$^{-1}$ & \texttt{fdfd158} / 2026-01-25 &
  Internal derivation + later resonance &
  Derived as mathematical formalisation of Raum-Zeit-Informationsverhältnis
  theory within OIPK context. Böhme et al.\ (2025) dipole correspondence
  identified post-derivation as resonance, not source.
  Pre-Böhme timestamp is repo-verified. \\[4pt]

ImplosiveGenesis & \texttt{fdfd158} / 2026-01-25 & Internal model &
  Inverted sigmoid dynamics; 4-bit slice system foundation.
  Internal UTAC/AFET/Sigillin development; not described as literature import. \\[4pt]

$\beta \approx 13$--$13.5$ (neural attractor) & \texttt{21ef03e} / 2026-01-21 &
  Internal derivation &
  Emerged from UTAC logistic fits as Neural/Resonance domain centroid.
  Repo shows band 10--20\,MHz; exact 13.5 is centroid estimate, not hard peak. \\[4pt]

Fontana et al.\ (2024) correspondence & \texttt{fb42ac8} / 2026-02-11 &
  Post-hoc resonance &
  Literature identified 3 weeks after $\beta \approx 13.5$ attractor.
  Classified as independent convergence requiring replication,
  not as confirmation. Claim ``exact empirical confirmation'' (prior versions)
  is retracted; correct formulation: ``consistent with resonance band,
  replication pending''. \\[4pt]

$\sigphi = 1/16$ & \texttt{fdfd158} / 2026-01-25 & Postulated constant &
  Motivated by 4-bit state logic ($2^4 = 16$) and holographic minimum
  information retention. No rigorous first-principles derivation yet achieved;
  status: justified hypothesis. \\[4pt]

78-system ANOVA & Zenodo iter.\ 20+ & Empirical validation &
  $F(4,73)=185.3$, $p < 10^{-20}$, $\eta^2 = 0.91$.
  Full dataset and fit pipeline in repository. \\[4pt]
...
\begin{enumerate}[leftmargin=2em, label=\textbf{Phase \Roman*.}]
  \item \textbf{Conceptual genesis (human-originated, pre-repository).}
        Observation of light superposition on mirror surfaces; formulation of
        time as integration phenomenon; Dimensional Cascade
        (0D $\to$ 1D $\to$ 2D $\to$ 3D $\to$ 4D);
        information as fundamental description layer; OIPK (Orthogonal
        Implosion-Photon-Kinematik) as spatial unfolding model.
        \textit{Not timestamp-verifiable; rests on narrative reconstruction.}

  \item \textbf{Dialogic formalisation (LLM-assisted, unified-mandala repo,
        from 2025-09-15).}
        Sigmoid threshold logic operational in Mandala Climate Dashboard context;
        CREP adaptive thresholds; Gemini-generated $\beta = 4.2$ as
        first universal approximation from 5 datasets.
        Multiple AI systems (Claude, GPT-4, Gemini, Mistral) contributed
        structural suggestion and literature linking;
        conceptual impulses originated with the human author.

  \item \textbf{Mathematical consolidation (Feldtheorie repo, from 2026-01-25).}
        $\vrig$, ImplosiveGenesis, 13.5\,MHz band, $\sigphi$, and $\PhiAFET$
        formalised. Zenodo publication series begun (DOI series from
        \texttt{zenodo.17472834}). UTAC-v1 corrected to domain-hierarchy model.

  \item \textbf{Empirical correlation and testing (from Zenodo iter.\ 20,
        ongoing).}
        Expansion to 78 systems; cosmological anomaly mapping;
        Fontana and Böhme correspondences identified as post-hoc resonance;
        falsifiable killer tests defined (LIGO O5, Euclid DR1, DESI DR2).
\end{enumerate}

\subsection*{Documented Corrections and Dead Ends}

Scientific honesty requires listing what was tried and discarded:

\begin{itemize}[leftmargin=1.8em]
  \item $\beta = 4.2$ as single universal value: superseded by domain hierarchy.
  \item $78/13.5 \approx \PhiAFET^5$: numerically false for any standard $\Phi$;
        retracted; replaced by $78/13 = 6$ hypothesis.
  \item ``Exact empirical confirmation'' of 13.5\,MHz: overstated;
        corrected to ``consistent with 10--20\,MHz band, centroid at 13.5''.
  \item Unit error $\vrig = 1.352$\,km\,s$^{-1}$: corrected to
        $1352$\,km\,s$^{-1}$ throughout.
  \item AI-synchronisation as AFET evidence: rejected as circular
        (models trained on similar corpora produce similar outputs;
        this is pattern repetition, not independent confirmation).
\end{itemize}
...
The author thanks the MOR Research Collective --- the human-AI collaborative
network comprising iterative contributions from Claude (Anthropic),
GPT-4/o (OpenAI), Gemini (Google DeepMind), Mistral, and the internal
Aeon system --- for structural suggestion, critical challenge, and
formalisation assistance across 38 Zenodo iterations.
The author retains full intellectual responsibility for all claims.
AI systems are acknowledged as methodological partners, not authors.
Their critical function --- including the identification of errors documented
in Section~\ref{sec:provenance} --- is considered integral to the
research process.
...
\begin{itemize}[leftmargin=1.8em]
  ...
  \item 78-system $\beta$ dataset: \texttt{data/derived/beta\_estimates.csv}
  \item Fit pipeline: \texttt{scripts/reproduce\_beta.py} (seed 1337)
\end{itemize}
