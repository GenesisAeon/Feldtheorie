...
def test_archiver_loads_existing_archive_index(tmp_path):
    ...
    existing_index = {"meta": {"system": "Test System"}, "archives": [{"name": "test_archive"}]}
...
def test_check_sigillin_size_detects_entry_count_exceeded_yaml(tmp_path):
    ...
    data = {"entries": [{"id": i} for i in range(10)]}  # 10 entries
...
def test_check_sigillin_size_within_limits(tmp_path):
    ...
    data = {"entries": [{"id": i} for i in range(5)]}  # 5 entries
    with open(test_file, "w", encoding="utf-8") as f:
        yaml.dump(data, f)
...
def test_archive_sigillin_keeps_recent_entries(tmp_path):
    ...
    test_file = tmp_path / "test.yaml"
    data = {"entries": [{"id": i} for i in range(10)]}
    with open(test_file, "w", encoding="utf-8") as f:
        yaml.dump(data, f)
...
def test_archive_sigillin_skips_when_within_limits(tmp_path):
    ...
    # Create small file
    test_file = tmp_path / "small.yaml"
    data = {"entries": [{"id": i} for i in range(5)]}
    with open(test_file, "w", encoding="utf-8") as f:
        yaml.dump(data, f)
...
def test_archive_sigillin_dry_run_does_not_modify(tmp_path):
    ...
    archiver = SigillinArchiver(base_path=tmp_path, max_entries=5, keep_recent=2, dry_run=True)

    test_file = tmp_path / "test.yaml"
    original_data = {"entries": [{"id": i} for i in range(10)]}
    with open(test_file, "w", encoding="utf-8") as f:
...
def test_save_archive_index_creates_trilayer(tmp_path):
    ...
    # Verify YAML content
    with open(archiver.archive_index_yaml, encoding="utf-8") as f:
        yaml_data = yaml.safe_load(f)
    assert yaml_data["meta"]["total_archives"] == 1

    # Verify JSON content
    with open(archiver.archive_index_json, encoding="utf-8") as f:
...
def test_collect_files_excludes_names(tmp_path):
    ...
    files = archiver._collect_files(tmp_path, patterns=["*.py"], exclude_names={"exclude_me.py"})
...
def test_collect_files_filters_by_suffix(tmp_path):
    ...
    files = archiver._collect_files(tmp_path, patterns=["*"], include_suffixes={".py", ".md"})
