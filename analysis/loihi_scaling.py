...
class SystemScaling:
    ...
    alpha: Optional[float] = None  # Skalierungsexponent (E ∝ N^α)
    beta_eff: Optional[float] = None  # Effektiver β-Wert
    kappa: Optional[float] = None  # Kopplungs-Index κ = β_eff / β_bio
...
def plot_scaling_laws(systems: List[SystemScaling], save_path: str = "analysis/plots/loihi_scaling_laws.png"):
    ...
    for i, system in enumerate(systems):
        ax1.loglog(system.N_values, system.E_values, 'o-',
                   color=colors[i % len(colors)], label=f"{system.name} (α={system.alpha:.2f})",
    ...
    ax1.plot(N_ref, 1e-10 * N_ref**0.75, '--', color='gray', alpha=0.5,
             linewidth=2, label='α = 0.75 (Kleiber)')
    ax1.plot(N_ref, 1e-10 * N_ref**1.0, ':', color='gray', alpha=0.5,
             linewidth=2, label='α = 1.0 (Linear)')
...
def main():
    ...
    for system in systems:
        ...
        print(f"  • {system.name:30s}: α = {system.alpha:.3f}, β_eff = {system.beta_eff:.2f}, κ = {system.kappa:.3f}")
