...
def bootstrap_beta_estimates(
    beta: np.ndarray, beta_ci_lower: np.ndarray, beta_ci_upper: np.ndarray, n_iterations: int = 1000
) -> dict:
...
def jackknife_analysis(beta: np.ndarray, domains: list[str]) -> dict:
    ...
    influential_systems = [
        (domains[i], float(beta[i]), float(influence_scores[i])) for i in influential_indices
    ]
...
def coefficient_stability_test(
    beta: np.ndarray, n_subsamples: int = 100, subsample_fraction: float = 0.8
) -> dict:
...
def main():
    ...
    results["bootstrap"] = bootstrap_beta_estimates(
        beta, beta_ci_lower, beta_ci_upper, args.n_bootstrap
    )
    if args.verbose:
        print(
            f"  Bootstrap mean: {results['bootstrap']['global_mean']:.2f} ± {results['bootstrap']['global_std']:.2f}"
        )
        print(
            f"  Bootstrap 95% CI: [{results['bootstrap']['global_ci_lower']:.2f}, {results['bootstrap']['global_ci_upper']:.2f}]"
        )
    ...
    if args.verbose:
        print(
            f"  Variance ratio (between/within): {results['field_type_stability']['variance_ratio']:.3f}"
        )
    ...
    results["coefficient_stability"] = coefficient_stability_test(
        beta, n_subsamples=100, subsample_fraction=0.8
    )
    if args.verbose:
