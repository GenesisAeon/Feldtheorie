...
def visualize_results(df, ft_results, dim_results, output_dir):
    ...
    fig.suptitle(
        "β-Scaling Follow-up Analysis: Post-Φ Falsification", fontsize=16, fontweight="bold"
    )
    ...
    for i, ft in enumerate(field_types):
        ...
        if data["n"] > 0:
            ...
            ax.barh(i, beta_range[1] - beta_range[0], left=beta_range[0], alpha=0.6)
            ax.text(
                beta_range[1] + 0.5,
                i,
    ...
    if "D_eff" in df.columns:
        for ft in df["field_type"].unique():
            ...
            ax.scatter(subset["D_eff"], subset["beta"], label=ft[:12], alpha=0.7, s=80)
        ...
        ax.plot(
            x_line, y_line, "k--", alpha=0.5, label=f"Linear fit: β={slope:.2f}·D+{intercept:.2f}"
        )
    ...
    if "C_eff" in df.columns:
        for ft in df["field_type"].unique():
            subset = df[df["field_type"] == ft]
            ax.scatter(subset["C_eff"], subset["beta"], label=ft[:12], alpha=0.7, s=80)
    ...
    colors = ["green" if d < 0.02 else "orange" if d < 0.05 else "red" for d in diffs]

    bars = ax.barh(constants, values, color=colors, alpha=0.6)
    ax.axvline(1.18, color="blue", linestyle="--", linewidth=2, label="Observed (1.18)")
    ax.set_xlabel("Value")
    ...
    if correlations:
        colors_corr = [
            "green" if abs(r) > 0.5 else "orange" if abs(r) > 0.3 else "red" for r in correlations
        ]
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="analysis/results", help="Output directory for results"
    )
    parser.add_argument(
        "--observed-ratio", type=float, default=1.18, help="Observed mean growth factor"
    )
    ...
    sig_correlations = [
        cov
        for cov, res in dim_results.items()
