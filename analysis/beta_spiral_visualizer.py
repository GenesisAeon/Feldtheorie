...
def plot_sigmoid_comparison(output_dir: Path):
    ...
    fig.suptitle(
        "UTAC v1.3φ: Classical vs Implosive Sigmoid Comparison", fontsize=14, fontweight="bold"
    )
    ...
    for idx, beta in enumerate(beta_values):
        ...
        ax.set_title(
            f"β = {beta:.3f}" + (" (Φ³ attractor)" if np.isclose(beta, PHI**3, rtol=0.01) else "")
        )
        ax.legend(loc="best")
...
def plot_3d_spiral(beta_sequence: np.ndarray, output_dir: Path):
    ...
    theta_spiral = (
        theta_values[0]
        + (theta_values[-1] - theta_values[0]) * t / (4 * np.pi)
        + 0.1 * np.cos(3 * t)
    )
    ...
    ax.set_title(
        "Φ^(1/3) Scaling Spiral in 3D Parameter Space\n(UTAC v1.3φ Type-6 Implosive Field)",
        fontsize=13,
        fontweight="bold",
    )
...
def plot_energy_release(beta_values: list, output_dir: Path):
    ...
    fig.suptitle(
        "Energy Release Integral E(R) for Implosive Fields", fontsize=14, fontweight="bold"
    )
...
def plot_trajectory_comparison(results: dict, output_dir: Path):
    ...
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("UTAC v1.3φ: Membrane Dynamics Time Evolution", fontsize=14, fontweight="bold")
...
def generate_summary_figure(results: dict, output_dir: Path):
    ...
    fig.suptitle(
        "UTAC v1.3φ Type-6 Implosive Origin Field - Complete Summary",
        fontsize=16,
        fontweight="bold",
    )
...
def main():
    """Main execution."""
    parser = argparse.ArgumentParser(description="UTAC v1.3φ Beta Spiral Visualizer")
    parser.add_argument(
        "--input",
        type=str,
