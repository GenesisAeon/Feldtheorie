...
def main():
    ...
    print(
        f"Significant (α=0.05): {'YES ✅' if result['hypothesis_test']['significant'] else 'NO ❌'}"
    )
    ...
    print("\n--- CONCLUSION ---")
    print(
        f"Φ-Scaling Detected: {'YES ✅' if result['conclusion']['phi_scaling_detected'] else 'NO ❌'}"
    )
    print(
        f"  - Mean ratio ≈ Φ: {'YES ✅' if result['conclusion']['mean_ratio_close_to_phi'] else 'NO ❌'}"
    )
    print(
        f"  - Strong log-regression: {'YES ✅' if result['conclusion']['log_regression_strong'] else 'NO ❌'}"
    )
    ...
    if args.plot:
        ...
        ax1.plot(
            x,
            result["log_regression"]["slope"] * x + np.log(beta_sorted[0]),
            "r--",
            linewidth=2,
            label=f'Fit: slope={result["log_regression"]["slope"]:.3f}',
        )
        ax1.plot(
            x,
            result["log_regression"]["expected_slope_log_phi"] * x + np.log(beta_sorted[0]),
            "g:",
            linewidth=2,
            label=f'Φ-Scaling: slope={result["log_regression"]["expected_slope_log_phi"]:.3f}',
        )
        ax1.set_xlabel("Index (n)", fontsize=12)
        ax1.set_ylabel("log(β)", fontsize=12)
        ax1.set_title(
            f"Φ-Scaling Test: Log-Regression\nR²={result['log_regression']['r_squared']:.3f}",
            fontsize=13,
        )
