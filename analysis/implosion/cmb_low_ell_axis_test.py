...
class CMBAxesTest:
    ...
    def _generate_random_alignments(self, n: int) -> np.ndarray:
        ...
        cos_angle = np.sin(theta1) * np.sin(theta2) * np.cos(phi1 - phi2) + np.cos(theta1) * np.cos(
            theta2
        )
    ...
    def run_full_analysis(self) -> dict:
        ...
        print(
            f"  Galactic coords: (l, b) = ({axis_dir['galactic_longitude_deg']:.1f}°, {axis_dir['galactic_latitude_deg']:.1f}°)"
        )
        ...
        type6_support = (
            quad_result["significant"]
            or axis_result["significant"]
            or spectrum_result["systematic_deficit"]
        )
        ...
        results = {
            ...
            "overall_verdict": {
                ...
                "summary": (
                    "CMB anomalies consistent with implosive origin"
                    if type6_support
                    else "CMB consistent with isotropic ΛCDM"
                ),
            },
        }
    ...
    def plot_results(self, results: dict, output_path: str = "paper/figures/cmb_axis_test.png"):
        ...
        fig.suptitle(
            "CMB Low-ℓ Anomaly Test - Type-6 Implosive Origin", fontsize=16, fontweight="bold"
        )
        ...
        ax1.plot(
            ell, self.C_ell_observed, "ro-", label="Observed (Planck)", markersize=5, alpha=0.7
        )
        ax1.fill_between(
            ell,
            self.C_ell_theory * 0.9,
            self.C_ell_theory * 1.1,
            alpha=0.2,
            color="gray",
        ...
        ax3.text(
            ...
            bbox=dict(
                boxstyle="round",
                facecolor="lightcoral" if p_axis < 0.05 else "lightgray",
                alpha=0.5,
            ),
        )
