...
class EarlyGalaxySpeedTest:
    ...
    def __init__(self, catalog_path: str = "data/implosion/cosmology_catalog.csv"):
        ...
        self.early_systems = self.catalog[
            (self.catalog["domain"] == "early_structure") & (self.catalog["redshift"] > 10)
        ].copy()
    ...
    def lambda_cdm_formation_rate(
        self, age_gyr: np.ndarray, rate_0: float, tau: float
    ) -> np.ndarray:
    ...
    def type6_accelerated_formation(
        self, age_gyr: np.ndarray, rate_0: float, tau: float, acceleration: float
    ) -> np.ndarray:
    ...
    def fit_lambda_cdm(self) -> dict:
        ...
        if "stellar_mass_log" in self.early_systems["observable"].values:
            mass_proxy = self.early_systems[self.early_systems["observable"] == "stellar_mass_log"][
                "value"
            ].values
        else:
    ...
    def run_full_analysis(self) -> dict:
        ...
        print(
            f"  Redshift range: {self.early_systems['redshift'].min():.1f} - {self.early_systems['redshift'].max():.1f}"
        )
        print(
            f"  Age range: {self.early_systems['age_myr'].min():.0f} - {self.early_systems['age_myr'].max():.0f} Myr"
        )
        ...
        if type6_result["fit_success"]:
            ...
            print(
                f"  γ = {type6_result['acceleration']:.3f} ± {type6_result['acceleration_err']:.3f}"
            )
            print(
                f"  Acceleration significant: {type6_result['acceleration_significant']} (p={type6_result['acceleration_pvalue']:.4f})"
            )
        ...
        type6_support = (comparison["preferred_model"] == "Type-6 Accelerated") or (
        ...
        results = {
            ...
            "overall_verdict": {
                ...
                "summary": (
                    "Early structure formation shows acceleration"
                    if type6_support
                    else "ΛCDM hierarchical assembly sufficient"
                ),
            },
        }
    ...
    def plot_results(
        self, results: dict, output_path: str = "paper/figures/early_galaxy_speed_test.png"
    ):
        """Create comprehensive visualization."""
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        fig.suptitle("Early Galaxy Formation Speed: Type-6 vs ΛCDM", fontsize=16, fontweight="bold")
        ...
        if lcdm["fit_success"]:
            ...
            ax1.plot(age_fine, mass_lcdm, "b--", linewidth=2, label=f"ΛCDM (AIC={lcdm['aic']:.1f})")

        if type6["fit_success"]:
            mass_type6 = self.type6_accelerated_formation(
                age_fine, type6["rate_0"], type6["tau_gyr"], type6["acceleration"]
            )
            ax1.plot(
                age_fine, mass_type6, "r-", linewidth=2, label=f"Type-6 (AIC={type6['aic']:.1f})"
            )
        ...
        ax2 = axes[0, 1]
        scatter = ax2.scatter(
            self.early_systems["redshift"],
            self.early_systems["age_myr"],
            c=self.early_systems["beta_estimate"],
            cmap="viridis",
            s=100,
            alpha=0.7,
            edgecolors="k",
        ...
        # Highlight GN-z11
        gn_z11 = self.catalog[self.catalog["system_id"] == "cosmo_001"].iloc[0]
        ax2.plot(gn_z11["redshift"], gn_z11["age_myr"], "r*", markersize=20, label="GN-z11 (O III)")
