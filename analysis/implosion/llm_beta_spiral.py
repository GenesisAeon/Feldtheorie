...
def load_llm_data(filepath: str) -> pd.DataFrame:
    ...
    if df["grokking_event"].dtype == bool or df["grokking_event"].dtype == object:
        df["grokking_event"] = (
            df["grokking_event"].astype(str).str.lower().isin(["true", "1", "yes"]).astype(int)
        )
...
def test_spiral_coherence(df: pd.DataFrame, verbose: bool = True) -> dict:
...
def test_grokking_beta_jumps(df: pd.DataFrame, verbose: bool = True) -> dict:
    ...
    for run_id in df["run_id"].unique():
        ...
        if grok_mask.sum() > 0:
            ...
            mean_jump_nogrok = (
                np.mean(np.abs(beta_jumps[~grok_mask])) if (~grok_mask).sum() > 0 else 0.0
            )
        else:
            mean_jump_grok = 0.0
    ...
    if verbose:
        ...
        print(
            f"  Jump amplification: {mean_jump_grok / mean_jump_nogrok:.2f}x"
            if mean_jump_nogrok > 0
            else "  Jump amplification: N/A"
        )
        print()
...
def test_fixpoint_convergence(df: pd.DataFrame, verbose: bool = True) -> dict:
...
def test_implosive_delay(df: pd.DataFrame, verbose: bool = True, n_bootstrap: int = 1000) -> dict:
    ...
    try:
        ...
        if verbose:
            ...
            print(
                f"    a (1/β coef): {a_fit:.3f} [{bootstrap_ci['a'][0]:.3f}, {bootstrap_ci['a'][1]:.3f}]"
            )
            print(
                f"    b (log coef): {b_fit:.3f} [{bootstrap_ci['b'][0]:.3f}, {bootstrap_ci['b'][1]:.3f}]"
            )
            print(
                f"    c (intercept): {c_fit:.3f} [{bootstrap_ci['c'][0]:.3f}, {bootstrap_ci['c'][1]:.3f}]"
            )
...
def cross_validate_tests(df: pd.DataFrame, k_folds: int = 5, verbose: bool = True) -> dict:
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(run_ids)):
        ...
        if not np.isnan(result4["r_squared"]):
            ...
            if verbose:
                print(
                    f"    Test 4 (R²): {result4['r_squared']:.3f}, ΔAIC: {result4['delta_aic']:.1f}"
                )
    ...
    for key, values in fold_results.items():
        if len(values) > 0:
            ...
            cv_summary[f"cv_{key}"] = (
                np.std(values) / np.mean(values) if np.mean(values) != 0 else np.nan
            )
        else:
    ...
    if verbose:
        ...
        print("═" * 70)
        print(
            f"  Test 1 (Autocorr): {cv_summary['mean_test1_autocorr']:.3f} ± {cv_summary['std_test1_autocorr']:.3f}"
        )
        print(
            f"  Test 2 (Grokking Corr): {cv_summary['mean_test2_corr']:.3f} ± {cv_summary['std_test2_corr']:.3f}"
        )
        print(
            f"  Test 3 (Final β): {cv_summary['mean_test3_beta']:.3f} ± {cv_summary['std_test3_beta']:.3f}"
        )
        print(
            f"  Test 4 (R²): {cv_summary['mean_test4_r2']:.3f} ± {cv_summary['std_test4_r2']:.3f}"
        )
        print(
            f"  Test 4 (ΔAIC): {cv_summary['mean_test4_delta_aic']:.1f} ± {cv_summary['std_test4_delta_aic']:.1f}"
        )
        print()
...
def create_spiral_plots(
    df: pd.DataFrame, spiral_result: dict, grokking_result: dict, output_path: str
):
    ...
    for run_id in df["run_id"].unique()[:5]:
        run_data = df[df["run_id"] == run_id].sort_values("epoch")
        ax.plot(run_data["epoch"], run_data["beta"], linewidth=2, alpha=0.6, label=f"Run {run_id}")
    ...
    if all_jumps_grok and all_jumps_nogrok:
        ...
        ax.hist(
            all_jumps_grok, bins=bins, alpha=0.8, color="red", label="Grokking events", density=True
        )

        ax.set_xlabel("|Δβ| (Jump Magnitude)", fontsize=11)
        ax.set_ylabel("Density", fontsize=11)
        ax.set_title(
            f'C: β-Jump Distribution (Corr={grokking_result["mean_correlation"]:.2f})',
            fontsize=12,
            fontweight="bold",
        )
    ...
    for run_id in df["run_id"].unique()[:5]:
        ...
        ax.plot(run_data["epoch"], cumulative_mean, linewidth=2, alpha=0.6, label=f"Run {run_id}")
...
def main():
    ...
    parser.add_argument(
        "--input",
        default="data/implosion/llm_runs_beta.csv",
        help="Path to LLM training β trajectories CSV",
    )
    parser.add_argument(
        "--out",
        default="paper/figures/llm_beta_spiral.png",
    ...
    parser.add_argument(
        "--k-folds", type=int, default=5, help="Number of folds for cross-validation (default: 5)"
    )
    ...
    if delay_result["falsified"] == False:
        print(
            f"  ✓ VALIDATED: R²={delay_result['r_squared']:.3f}, ΔAIC={delay_result['delta_aic']:.1f}"
        )
        ...
        if "falsification_details" in delay_result:
            ...
            if details.get("aic"):
                print("      - Poor model fit (ΔAIC < 4)")
            if details.get("coef_a"):
                print("      - 1/β coefficient not significant")
            if details.get("r2"):
                print("      - R² < 0.3")
    else:
    ...
    if cv_result and "n_folds_completed" in cv_result:
        print(
            f"  Cross-validation: {cv_result['n_folds_completed']}/{cv_result['n_folds']} folds completed"
        )
    print()
